...
export default class Scrollbar extends React.PureComponent {
  ...
  handlerContainerScroll = e => {
    ...
    let el = this.el
    let top = el.scrollTop / (el.scrollHeight - this.offsetHeight) * (1 - this.offsetHeight / this.lastScrollHeight) * 100
    let bottom = (1 - (el.scrollTop + this.offsetHeight) / (el.scrollHeight - this.offsetHeight) * (1 - this.offsetHeight / this.lastScrollHeight)) * 100
    if (bottom < 0) bottom = 0
    let left = el.scrollLeft / (el.scrollWidth - this.offsetWidth) * (1 - this.offsetWidth / this.lastScrollWidth) * 100
    let right = (1 - (el.scrollLeft + this.offsetWidth) / (el.scrollWidth - this.offsetWidth) * (1 - this.offsetWidth / this.lastScrollWidth)) * 100
    if (right < 0) right = 0
    let pos = {
      top: top,
      bottom: bottom,
  ...
  handleHandlerMouseMove = e => {
    if (this.activeHandler === VERTICAL) {
      let delY = e.clientY - this.lastMousePos.top
    ...
    if (this.activeHandler === HORIZONTAL) {
      let delX = e.clientX - this.lastMousePos.left
